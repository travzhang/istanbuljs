...
import type { Location, Range } from "@vitest/istanbul-lib-coverage";

import * as pathutils from "./pathutils";

/** an original source position as resolved through a source map */
...
/** the result of mapping a generated range back to an original source */
 ...
 */
function originalEndPositionFor(
  sourceMap: TraceMap,
  generatedEnd: Location,
  ...
  if (beforeEndMapping.source === null) {
    ...
    for (let line = generatedEnd.line; line > 0 && beforeEndMapping.source === null; line--) {
      beforeEndMapping = originalPositionTryBoth(sourceMap, line, Infinity);
    }
    if (beforeEndMapping.source === null) {
      return null;
    }
  ...
  const afterEndMappings: (GeneratedMapping | InvalidGeneratedMapping)[] = allGeneratedPositionsFor(
    sourceMap,
    {
...
function isInvalidPosition(pos: Partial<Location> | undefined): boolean {
  return (
    !pos ||
 ...
 * @param generatedLocation the original location Object
 * @returns the remapped location Object
 */
export default function getMapping(
  sourceMap: TraceMap,
  generatedLocation: Range | null | undefined,
  origFile: string,
): Mapping | null {
  ...
  let end: OriginalPosition | null = originalEndPositionFor(sourceMap, generatedLocation.end);
