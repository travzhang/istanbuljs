 ...
 */
import type { FileCoverage, FunctionMapping, Range } from "@vitest/istanbul-lib-coverage";
...
/** allows the optional `skip` marker some instrumenters place on metadata */
type MaybeSkipped<T> = T & { skip?: boolean };

/** a single line of source text being annotated */
...
/** an object that can look up source code for a file path */
export interface SourceStore {
  getSource(filePath: string): string;
}

/** per-line coverage info as rendered in the detail view */
export interface LineCoverageInfo {
  covered: string | null;
  hits: number | string;
}

/** the annotated source data consumed by the html report's detail template */
export interface AnnotatedData {
  annotatedCode: string[];
  lineCoverage: LineCoverageInfo[];
  maxLines: number;
}
...
function annotateLines(fileCoverage: FileCoverage, structuredText: StructuredTextItem[]): void {
  const lineStats = fileCoverage.getLineCoverage();
  if (!lineStats) {
    return;
  }
  Object.entries(lineStats).forEach(([lineNumber, count]) => {
    const item = structuredText[lineNumber as unknown as number];
    if (item) {
      item.covered = count > 0 ? "yes" : "no";
      item.hits = count;
    }
  });
}

function annotateStatements(
  fileCoverage: FileCoverage,
  structuredText: StructuredTextItem[],
): void {
  const statementStats = fileCoverage.s;
  const statementMeta = fileCoverage.statementMap;
  Object.entries(statementStats).forEach(([stName, count]) => {
    const meta = statementMeta[stName] as MaybeSkipped<Range>;
...
function annotateFunctions(fileCoverage: FileCoverage, structuredText: StructuredTextItem[]): void {
  ...
  Object.entries(fnStats).forEach(([fName, count]) => {
    const meta = fnMeta[fName] as MaybeSkipped<FunctionMapping>;
    const type = count > 0 ? "yes" : "no";
...
function annotateBranches(fileCoverage: FileCoverage, structuredText: StructuredTextItem[]): void {
  ...
  Object.entries(branchStats).forEach(([branchName, branchArray]) => {
    ...
    let i: number;
    let count: number;
    let meta: MaybeSkipped<Range>;
    let startCol: number;
    let endCol: number;
    let startLine: number | undefined;
    let endLine: number | undefined;
    let openSpan: string;
    let closeSpan: string;
    let text: InsertionText;
    ...
    if (sumCount > 0 || (sumCount === 0 && branchArray.length === 1)) {
      ...
      for (i = 0; i < branchArray.length && i < metaArray.length; i += 1) {
        ...
        if (count === 0 && structuredText[startLine as number]) {
          //skip branches taken
          if (endLine !== startLine) {
            endCol = structuredText[startLine as number].text.originalLength();
          }
          text = structuredText[startLine as number].text;
...
function annotateSourceCode(fileCoverage: FileCoverage, sourceStore: SourceStore): AnnotatedData {
  let codeArray: string[];
  let lineCoverageArray: LineCoverageInfo[];
  try {
    const sourceText = sourceStore.getSource(fileCoverage.path);
    const code = sourceText.split(/(?:\r?\n)|\r/);
    let count = 0;
    const structured: StructuredTextItem[] = code.map((str) => {
      count += 1;
      return {
        line: count,
        covered: "neutral",
    ...
    codeArray = [(ex as Error).message];
    lineCoverageArray = [{ covered: "no", hits: 0 }];
    String((ex as Error).stack || "")
      .split(/\r?\n/)
      .forEach((line) => {
        codeArray.push(line);
