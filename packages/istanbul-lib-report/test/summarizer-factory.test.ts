...
import SummarizerFactory from "../src/summarizer-factory";
import type { ReportNode, ReportTree } from "../src/summarizer-factory";
import * as coverageMap from "./helpers/coverage-map";

function getStructure(tree: ReportTree, localNames?: boolean): string[] {
  const meth = localNames ? ("getRelativeName" as const) : ("getQualifiedName" as const);
  const visitor = {
    nodes: [] as string[],
...
describe("summarizer", () => {
  let fn: (map: CoverageMap) => ReportTree;
