...
import { SHA, MAGIC_KEY, MAGIC_VALUE } from "./constants.mjs";
import { getIgnoredLines } from "./ignored-lines.mjs";
  ...
  insertCounter(path, increment) {
    ...
    if (path.isBlockStatement()) {
      ...
      if (parent && T.isExportNamedDeclaration(parent.parentPath)) {
        ...
        parent &&
        (T.isProgram(parent.parentPath) || T.isBlockStatement(parent.parentPath))
      ) {
...
function shouldIgnoreFile(programNode) {
  return (
    programNode.parent && programNode.parent.comments.some((c) => COMMENT_FILE_RE.test(c.value))
  );
}
